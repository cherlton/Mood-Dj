...
export default function PlaylistResult({ mood = "happy", tracks = ["spotify:track:4iV5W9uYEdYUVa79Axb7Rh", "spotify:track:1Je1IMUlBXcx1Fz0WE7oPT", "spotify:track:6f70bfcMKEyc4DsCI5e8SI"] }) {
  ...
  // Get current time and set theme (same logic as MoodForm)
  useEffect(() => {
    ...
    setTimeout(() => setShowTracks(true), 500);
  ...
  useEffect(() => {
    if (tracks && tracks.length > 0) {
      ...
      setNowPlayingTrack(tracks[0]); // Set first track as now playing
      // Create shuffled version
      const shuffled = [...tracks].sort(() => Math.random() - 0.5);
      setShuffledTracks(shuffled);
      
      // Initialize iframe refs
  ...
  const controlIframePlayback = (action) => {
    // Try to send postMessage to current iframe
  ...
  const handleShuffle = () => {
    ...
    if (!isShuffled) {
      // Enable shuffle - use shuffled tracks
      ...
      // Disable shuffle - use original order
      setDisplayTracks(tracks);
  ...
  const handleNext = () => {
    if (repeatMode === "one") {
      // Stay on current track but restart
  ...
  // Theme configurations (matching MoodForm)
  const themes = {
    morning: {
                ...
                >
                  <div className="relative overflow-hidden rounded-xl">
                    <iframe
                      ...
                      src={`https://open.spotify.com/embed/track/${uri.split(":")[2]}?utm_source=generator&theme=${theme === 'night' ? '0' : '1'}&autoplay=0&show_cover_art=true`}
                      width="100%"
                      height="152"
                      frameBorder="0"
